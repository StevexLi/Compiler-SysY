...
public class LoopInfo {
    ...
    private final ArrayList<IRLoop> loops = new ArrayList<>();
    private final ArrayList<IRLoop> topLevelLoops = new ArrayList<>();
    ...
    public void computeLoopInfo(Function f) {
        Ir.optimization.DomAnalysis.analyzeDom(f);
    ...
    public void computeIndVar() {
        for (IRLoop l : loops) {
            ...
            if (!(l.getIndVarCondInst() instanceof BinaryIns)) {
                return;
            }
            ...
            for (Value op : indVarCondInst.getOperands()) {
                if (op instanceof PhiIns) {
                    l.setIndVar((PhiIns) op);
                } else {
                    compareBias = op;
                }
            }
