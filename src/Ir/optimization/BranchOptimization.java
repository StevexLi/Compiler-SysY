...
import Ir.values.instructions.PhiIns;
...
public class BranchOptimization implements Pass.IRPass{
    ...
    public void run(IRModule m) {
        for (INode<Function, IRModule> funcEntry : m.getFunctions()) {
            if (!funcEntry.getValue().isLibFunc()) {
    ...
    private boolean delOnlyUncondBr(Function f) {
        ...
        while (bbNode != null) {
            ...
            Instruction brInst = bb.getIns().getEnd().getValue();

            if (bb.getIns().getSize() == 1 && brInst instanceof BrIns && brInst.getOperands().size() == 1) {
                boolean flag = false;
                BasicBlock succ = (BasicBlock) (brInst.getOperands().get(0));
                if (succ.getIns().getBegin().getValue() instanceof PhiIns) {
                ...
                for (BasicBlock pred : bb.getPredecessors()) {
                    Instruction predLastInst = pred.getIns().getEnd().getValue();
                ...
                for (INode<Instruction, BasicBlock> instrEntry : succ.getIns()) {
    ...
    private boolean mergeUnCondJump(Function f) {
        ...
        for (INode<BasicBlock, Function> bbEntry : f.getList()) {
            ...
            while (notOk) {
                notOk = false;
                Instruction brInst = bb.getIns().getEnd().getValue();
                if (brInst instanceof BrIns && brInst.getOperands().size() == 1) {
                    BasicBlock succ = (BasicBlock) brInst.getOperands().get(0);
                    if (!(succ.getIns().getBegin().getValue() instanceof PhiIns)) {
    ...
    public boolean removeSinglePhi(Function f) {
        ...
        for (INode<BasicBlock, Function> bbEntry : f.getList()) {
            BasicBlock bb = bbEntry.getValue();
            INode<Instruction, BasicBlock> instrEntry = bb.getIns().getBegin();
    ...
    private boolean mergeConstCondBr(Function f) {
        ...
        for (INode<BasicBlock, Function> bbNode : f.getList()) {
            ...
            Instruction brInst = bb.getIns().getEnd().getValue();
            if (brInst instanceof BrIns && brInst.getOperands().size() == 3) {
    ...
    private boolean mergeBasicBlock(BasicBlock pred, BasicBlock succ) {
        ...
        INode<Instruction, BasicBlock> instrEntry = succ.getIns().getBegin();
        while (instrEntry != null) {
            INode<Instruction, BasicBlock> tmp = instrEntry.getNext();
            instrEntry.removeFromList();
            instrEntry.insertBefore(pred.getIns().getEnd());
            instrEntry = tmp;
        }
        pred.setSuccessors(succ.getSuccessors());
        pred.getIns().getEnd().removeFromList();
